...
def mainscreen():                                                   #Erstellung der Funktion vom Hauptscreens
    ...
    welcome.place(x=80, y=80)                                       #Hier wird die Wilkomensschrift erstellt und im GUI platziert
    welcome.config()

    loginButton = Button(text="Login", width="30", command=login, style='log.TButton')      #Hier wird der Login Knopf erstellt mit dem Style der oben erstellt wurde und mit dem Kommand der später noch defeniert wird
    loginButton.place(x=90, y=290)
    loginButton.config()
    chatgui.bind('<Return>',lambda event:login())                   #Der .bind Kommand kann einer Taste auf dem Keyboard eine Funktion zu weisen
    ...
    chatgui.bind('<Shift_L><Return>', lambda event:quit())          #Diesesmal wird einer Tastenkombination eine Funktion zugewiesen

    chatgui.mainloop()                                              #Dieser Kommand ist sehr wichtig, da er den Mainscreen ausführt und immer wieder aktualiesiert
...
def userselection():                                                #Diese Funktion erzeugt das Chatfenster und die Eingabe
    ...
    chatdisplay.configure(cursor="arrow", state=DISABLED)           #Diese Zeile verhindert das man direkt etwas im Textfeld verändern kann
    ...
    scrollbar.config(command=chatdisplay.yview)                     #Dieser Kommand sorgt dafür, dass die Scrollbar funktioniert
    ...
    inputentry.focus()                                              #Der .focus Kommand sorgt dafür dass man dieses Feld nicht extra anklicken muss sondern das man direckt anfangen kann zu schreiben

    def delay():                                                    #Die delay-Funktion sorgt für die verzögern form schließen des Fensters
        ...
        chatgui.after(2500, quit)                                   #Diese Zeile löst die Zersttör-Funktion nach 2500ms aus

    exit2 = Button(text="Exit", width=10, command=delay, style='log.TButton')       #Exit Knopf der delay ausfürht wird erstellt
...
def register():                                                     #Diese Funktion regestriert die neuen User
    global username
    global password
    global username_entry                                           #Hier werden wieder globale Variablen festgelt damit alle funktionen damit arbeiten können
    global password_entry
    global registerScreen

    registerScreen = Toplevel(chatgui)                              #Hier wird ein Popup erstellt wo sich der User regestrieren kann
    ...
    Label(registerScreen, text="Please enter an username and a password!", background=bg_color1, foreground=txt_color).pack()       #Deises Label gibt dem User Anweisungen
    ...
    username_entry.pack()                                           #Der .pack Kommand plaziert das Element mittig
    ...
    password_entry = Entry(registerScreen, textvariable=password, show="*")         #Das Eingabefeld vom Password hat eine besonderheit, nähmlich die show-Eiganschaft, die in deisem Fall nur einen * zeigt für alles was der User in gibt
    password_entry.pack()                                           #Siehe Z.184
    Label(registerScreen, text="", background=bg_color1).pack()     #Siehe Z.181
    Button(registerScreen, text="Register", width="10", command=registerUser, style='log.TButton').pack()       #Alls letztes wird noch der Knopf erstellt der den regestrieungs Vorgang auslöst
    registerScreen.bind('<Return>', lambda event:registerUser())    #Siehe Z.60

def registerUser():                                                 #Diese Funktion regestriert den User
    if username.get() == "" or password.get() == "":                #Hier wird geschaut ob es einen Input gibt
        tkinter.messagebox.showinfo(title="Info", message="No input recognized!")       #Hier wird ein Info-Fenster erstellt was sagt das es kein Input gab
    else: 
        ...
        tkinter.messagebox.showinfo(title="Info", message="Registration succesful!")        #Diese Zeile erstellt ein Info-Fenster was die Regestrierung bestätigt
        registerScreen.destroy()                                    #Hier wird das Regestrierungsfenster zerstört

def login():                                                        #Diese Funktion kümmert sich um das Einloggen
    global loginScreen
    global username_verify
    global password_verify                                          #Zu erst werden wieder globale Variabln erstellt
    ...
    Label(loginScreen, text="", background=bg_color1).pack()        #Siehe Z.181
    Button(loginScreen, text="Login", width="10", command=loginUser, style='log.TButton').pack()        #Alls letztes wird noch der Knopf erstellt der den User einlogt
